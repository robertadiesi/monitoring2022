...
setwd("C:/Users/Rober/Desktop/exam_project_pirenei")
...
# Since the area visualized is too wide and I want to focus only on the area of the Pirenei, I'm going to crop the image 
# let's add the coordinates numbers I found on Copernicus to define the region of interest 
ext <- c(-1.60, 2.89, 41.70, 43.52)
...
png(file="image_winter22.png", units="cm", width=30, height=25, res=500)
plot(snow22)
dev.off()
...
# Save in PNG
png(file="image_winter17.png", units="cm", width=30, height=25, res=500)
plot(snow17)
dev.off()

# Using ggplot function with viridis
p22 <- ggplot() + geom_raster(snow22, mapping = aes(x=x, y=y, fill = Snow.Cover.Extent)) + scale_fill_viridis() + ggtitle("Snow Cover in 2022")
p17 <- ggplot() + geom_raster(snow17, mapping = aes(x=x, y=y, fill = Snow.Cover.Extent)) + scale_fill_viridis() + ggtitle("Snow Cover in 2017")

# I want to see both images to compare them 
p17/p22

# Save it in PNG
png(file="image_winter17_22.png", units="cm", width=30, height=25, res=500)
p17 / p22
dev.off()
...
png(file="diff_winter17_22.png", units="cm", width=30, height=25, res=500)
plot(dif, col=coldif)
dev.off()
